...
To view the interactive, live dashboard:
```bash
cd frontend
pnpm install
pnpm dev
```
Open http://localhost:5173 in your browser to view the premium dashboard. The dashboard reads the matched audit outputs from `frontend/public/data.json` generated by the Python engine.
...
CloudMesh contains automated deployment scripts to build the React frontend locally and synchronize the codebase to a remote server using SSH/rsync.
- **`deploy.sh`**: Bash script for Linux/macOS environments.
- **`deploy.ps1`**: PowerShell script for Windows environments (with an automatic fallback to `scp` if `rsync` is not installed on the system).

To deploy to your production server:
1. Open the script (`deploy.sh` or `deploy.ps1`) and configure the connection parameters:
   ```bash
   TARGET_IP="49.13.65.81"
   TARGET_USER="root"
   TARGET_DIR="/home/cloudmesh"
   ```
2. Make sure you have SSH key access configured to target the production server.
3. Run the script:
   - Bash: `./deploy.sh`
   - PowerShell: `.\deploy.ps1`

The script will:
1. Compile the React app dashboard locally.
2. Synchronize python files, configurations, and compiled frontend assets (`frontend/dist`) to the server.
3. Install/update python virtual environment dependencies.
4. Launch the telemetry stack and the Nginx dashboard container serving the static dashboard on port `8080`.
5. Trigger an initial audit run to generate the dashboard dataset.
